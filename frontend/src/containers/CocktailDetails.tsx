...
import { selectCocktail } from '../store/slices/cocktailSlice.ts';
import { useParams } from 'react-router-dom';
import React, { useEffect, useState } from 'react';
import { fetchCocktailById, rateCocktail } from '../store/thunks/cocktailThunk.ts';
import { Box, Card, CardContent, CardMedia, Typography, Rating, CircularProgress } from '@mui/material';
  ...
  return (
    <Box display="flex" justifyContent="center" alignItems="center" mt={4}>
      <Card sx={{ maxWidth: 600, boxShadow: 3, borderRadius: 3, padding: 3 }}>
        <CardMedia component="img" height="300" image={`http://localhost:8000/public/${cocktail.image}`} alt={cocktail.title} />
        <CardContent sx={{display: 'flex', flexDirection: 'column'}}>
          ...
          <Typography variant="h6" mt={2} sx={{mt: 5, alignSelf: 'center'}}>Rating:</Typography>
          <Rating
